...
@router.route('/color_err')
async def color_err(ws, path):
    try:
        while True:
            try:
...
async def sql_color_err(data):
    ...
    sql = 'UPDATE sim_orders SET status = %s, err_color = %s WHERE num = %s AND item_id = %s'
    val = (status, err_color, num, item_id)
