...
async def sql_item_err_handler(data):
    ...
    if err_cause == 'clear':
        ...
        sql = 'SELECT * FROM sim WHERE category = %s AND name = %s AND color = %s AND producer = %s ORDER by fifo'
        value = (category, name, color, producer)
        connect.ping(reconnect=True)
        ...
        del_reserve = 'UPDATE sim SET reserve = reserve - %s WHERE id = %s'
        del_reserve_value = (order_quant, item_id)
        connect.ping(reconnect=True)
        cursor.execute(del_reserve, del_reserve_value)
        connect.commit()
