...
@router.route('/item_err')
async def item_err(ws, path):
    try:
        while True:
            try:
...
async def sql_item_err(data):
    ...
    sql = 'UPDATE sim_orders SET status = %s, err_item = %s WHERE num = %s AND item_id = %s'
    val = (status, err_item, num, item_id)
